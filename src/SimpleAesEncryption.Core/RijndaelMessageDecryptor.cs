...
namespace ArtisanCode.SimpleAesEncryption
{
    public class RijndaelMessageDecryptor : RijndaelMessageHandler, IMessageDecryptor
    {
        ...
        public virtual string Decrypt(string cypherText)
        {
            ...
            var primatives = cypherText.Split(new[] { CYPHER_TEXT_IV_SEPARATOR }, StringSplitOptions.RemoveEmptyEntries);

            if (primatives.Length != 2)
            {
                throw new ArgumentException("Invalid cypherText. Unable to determine the correct IV used for the encryption. Please ensure the source string is in the format 'Cypher Text'" + CYPHER_TEXT_IV_SEPARATOR + "'IV'", "source");
            }
